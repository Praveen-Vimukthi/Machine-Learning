...
### 2. Using Pandas Categorical Codes

Pandas can convert text categories into numbers without using external libraries like sklearn.

- Step 1: Convert the column to a categorical type using astype('category').
- Step 2: Use .cat.codes to get numbers for each category.
- Step 3: Use .cat.categories to see which number corresponds to which category.

**Important**: The numbers are assigned in alphabetical order of the categories.
...
# Convert Fruit column to categorical and get codes
data['Fruit_Code'] = data['Fruit'].astype('category').cat.codes

# Get category mapping
categories = data['Fruit'].astype('category').cat.categories

print(data)
print("Category Mapping:", list(categories))
```
**Output:**
```
     Fruit  Price  Fruit_Code
0    Apple    1.2           0
1   Banana    0.5           1
2   Orange    0.8           2
3    Apple    1.3           0
...
1. Pandas sees the `Fruit` column has 3 unique categories: Apple, Banana, Orange.

2. Alphabetically, it assigns numbers:
    - Apple → 0
    - Banana → 1
    - Orange → 2

3. `.cat.codes` replaces the text with numbers in a new column.

4. `.cat.categories` shows which number corresponds to which fruit.

---

### 3. Encoding Ordinal Data Manually
...
### 4. Handling Unseen Categories in Test Data

When we train a model, the LabelEncoder only “knows” the categories it saw during training.

- If new categories appear in test or deployment data, using transform() directly will cause an error.
- To avoid this, you can:
      1. Check if the category exists in `le.classes_`.
      2. If it doesn’t exist, assign a default value like `-1`.
- This ensures the model won’t crash when it sees new data.
...
# Training data
train_labels = ["Apple", "Banana", "Orange"]
le = LabelEncoder()
le.fit(train_labels)

# Test data with an unseen category
test_labels = ["Apple", "Banana", "Mango"]

# Encode safely
encoded_test = [le.transform([x])[0] if x in le.classes_ else -1 for x in test_labels]

print("Encoded Test Labels:", encoded_test)
```
**Output:**
```
Encoded Test Labels: [0, 1, -1]
```

#### Explanation

1. `le.fit(train_labels)` learns the categories: `['Apple', 'Banana', 'Orange']`.
    - Apple → 0
    - Banana → 1
    - Orange → 2

2. Test data contains `"Mango",` which was not in training.

3. Using the list comprehension:
```python
le.transform([x])[0] if x in le.classes_ else -1
```
- If the label exists in training, encode it.
- If it doesn’t, assign `-1`.

4. This gives `[0, 1, -1]` instead of throwing an error.
...
| Title                         | Link                                                 |
|------------------------------|------------------------------------------------------|
| 💻 Google Colab File         | [View File](https://colab.research.google.com/drive/1qrGO313YsrjxOd-JeJmwAsHkbPKHAUpl?authuser=2#scrollTo=9rMbjywIucJB)               |

<table width="100%">
  <tr>
    <td align="left">
      <a href="https://github.com/Praveen-Vimukthi/Machine-Learning/blob/main/Module_4/4.4%20Data%20Standardization.md">&larr; Previous</a>
    </td>
    <td align="right">
      <a href="README_STEP3.mdhttps://github.com/Praveen-Vimukthi/Machine-Learning/blob/main/Module_4/Train%20Test%20Split.md
  </tr>
</table> 
